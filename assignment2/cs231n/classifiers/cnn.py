...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        stride_conv, pad = 1, (filter_size-1) / 2
        stride_mp, pool_h, pool_w = 2,2,2

        h_convout = 1 + (H + 2 * pad - filter_size) / stride_conv
        w_convout = 1 + (W + 2 * pad - filter_size) / stride_conv
        h_mpout = int(1 + (h_convout - pool_h) / stride_mp)
        w_mpout = int(1 + (w_convout - pool_w) / stride_mp)
        ...
        self.params['W2'] = np.random.randn(h_mpout*w_mpout*num_filters, hidden_dim) * weight_scale
