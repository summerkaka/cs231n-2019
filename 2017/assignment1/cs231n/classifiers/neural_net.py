...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    #pass
    q1 = X.dot(W1) + b1  #(N, H)  'H' is hidden_size
...
#    scores = h_output.dot(W2) + b2
    ...
    loss = np.sum(loss_each_sample) / N + 0.5* reg* (np.sum(W1*W1) + np.sum(W2*W2))
    #from github
...
#    grads['b1'] = np.sum(dh_ReLu, axis = 0)
  ...
  def predict(self, X):
    ...
    ###########################################################################
    #pass
    scores = self.loss(X) #return scores when y == none
