...
def affine_backward(dout, cache):
    ...
    #pass
    x_local = x.reshape(x.shape[0], -1)
    dw = x_local.T.dot(dout)
    dx_local = dout.dot(w.T)
    dx = dx_local.reshape(x.shape)
    #db = np.ones(b.shape) xiatian: wrong, rel_error() would get result 1.0
    db = np.sum(dout, axis = 0)  #from github
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        #pass
        #xiatian: don't understand why operation by column
        mean = np.mean(x, axis=0)
        var = np.var(x, axis = 0)
        #var = np.sum((x - mean)**2, axis=0) / N 
        #print('x, mean， var shape and value', x.shape, mean.shape, var.shape, mean, var)
        x_local = (x - mean) / np.sqrt(var + eps)
        out = gamma * x_local + beta
        cache = (gamma, x, mean, var, eps, x_local)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            for k in range(F):
                out[:, k , i, j] = np.sum(x_pad_masked * w[k, :, :, :], axis=(1,2,3)) #out的第2维第k个结果存的是w的第k个滤波器的c个channel的卷积和
